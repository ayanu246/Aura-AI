...
if st.session_state.current_chat:
    ...
    for m in msgs:
        with st.chat_message(m["role"]):
            if m.get("is_image"): st.image(m["content"], caption="Generated by Aura AI")
    ...
    if p := st.chat_input("Ask or Draw..."):
        ...
        img_keywords = ["draw", "image", "photo", "picture", "generate", "make a"]
        is_img_req = any(word in p.lower() for word in img_keywords)

        if is_img_req:
            clean_p = p.lower()
            for w in img_keywords: clean_p = clean_p.replace(w, "")
            ...
            with st.chat_message("assistant"):
                st.write(f"🎨 Creating: **{clean_p}**")
                st.image(img_url)
            
            msgs.append({"role": "user", "content": p})
            msgs.append({"role": "assistant", "content": img_url, "is_image": True})
            st.rerun()
